(function () {
  ...
  function bindNav() {
    NA.$$(".admin-nav__item[data-page]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        P.navigate(btn.dataset.page).catch(function (e) {
          NA.toast(e.message || "Erro ao navegar", "error");
        });
      });
    });
    ...
    NA.$("#erp-dono-check").addEventListener("click", function () {
      P.checkErpDono();
    });
  ...
  function init() {
    NA.closeModal(NA.$("#confirm-modal"));
